...
(function() {
  ...
  app = angular.module('pistolSearch', ['ngRoute', 'ngAnimate', 'ngMaterial', 'gunData']);
  ...
  app.controller('IndexCtrl', [
    '$scope', '$rootScope', '$location', 'GUNS', 'UserSearch', 'CRITERIA', 'PRINTING', '$mdSidenav', '$mdDialog', function($scope, $rootScope, $location, GUNS, UserSearch, CRITERIA, PRINTING, $mdSidenav, $mdDialog) {
      $scope.showTabDialog = function(gun) {
        $scope.gun = _.find(GUNS, {
          'id': gun
        });
      ...
      $scope.checkExclusives = function(category, selection) {
        var index, member, _i, _len, _ref, _results;
        if (CRITERIA.categories[category].exclusive) {
          _ref = CRITERIA.categories[category].members;
          _results = [];
          for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
            member = _ref[index];
            if ($scope.selections.categories[category][index] === 'must' && index !== selection) {
              _results.push($scope.selections.categories[category][index] = 'can');
            } else {
      ...
      $scope.process = (function() {
        var caliberIndex, index, member, _i, _len, _ref;
        caliberIndex = _.findIndex($scope.criteria.categories, {
          'field': 'caliber'
        });
        _ref = $scope.criteria.categories[caliberIndex].members;
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          member = _ref[index];
          if ($scope.wizard.caliber[member] != null) {
            $scope.selections.categories[caliberIndex][index] = $scope.wizard.caliber[member];
          }
        }
        return $location.path('/');
      })();
      $scope.masterGun = function(input) {
        var category, index, index2, option, two, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
        _ref = $scope.selections.categories;
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          category = _ref[index];
          for (index2 = _j = 0, _len1 = category.length; _j < _len1; index2 = ++_j) {
            two = category[index2];
            switch (two) {
              case 'must':
                if (input[CRITERIA.categories[index].field] !== CRITERIA.categories[index].members[index2]) {
                  return false;
                }
                break;
              case 'cant':
        ...
        _ref1 = $scope.selections.options;
        for (index = _k = 0, _len2 = _ref1.length; _k < _len2; index = ++_k) {
          option = _ref1[index];
          switch (option) {
            case 'must':
              if (!_.includes(input.options, PRINTING.options[index].name)) {
                return false;
              }
              break;
            case 'cant':
      ...
      return $scope.reset = function() {
        var category, index, index2, option, two, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _results;
        _ref = $scope.selections.categories;
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          category = _ref[index];
          for (index2 = _j = 0, _len1 = category.length; _j < _len1; index2 = ++_j) {
        ...
        for (index = _k = 0, _len2 = _ref1.length; _k < _len2; index = ++_k) {
          option = _ref1[index];
          _results.push(delete $scope.selections.options[index]);
        }
        return _results;
      };
    }
  ...
  app.controller('WizardCtrl', [
    '$scope', '$rootScope', '$location', '$sce', 'GUNS', 'UserSearch', 'CRITERIA', 'WIZARD', function($scope, $rootScope, $location, $sce, GUNS, UserSearch, CRITERIA, WIZARD) {
      var prompt, prompts, _i, _len;
      $scope.wizard = UserSearch.wizard;
      ...
      */
      ...
      for (_i = 0, _len = prompts.length; _i < _len; _i++) {
        ...
        if (typeof prompt.description === 'string') {
          prompt.description = $sce.trustAsHtml(prompt.description);
        }
      }
      $scope.prompt = prompts[$scope.currentPrompt];
      return $scope.choice = function(cal, decision) {
        $scope.wizard.caliber[cal] = decision;
        $scope.currentPrompt++;
        if ($scope.currentPrompt < prompts.length) {
          return $scope.prompt = prompts[$scope.currentPrompt];
        } else {
          return delete $scope.prompt;
        }
      };
    }
  ]);
  ...
  app.controller('DetailCtrl', [
    '$scope', '$rootScope', '$routeParams', 'GUNS', 'PRINTING', '_', function($scope, $rootScope, $routeParams, GUNS, PRINTING, _) {
      $scope.gun = _.find(GUNS, {
        'id': $routeParams.id
      });
      $scope.details = PRINTING.detailPairs;
      return $scope.measurementUnits = PRINTING.measurementUnits;
    }
  ]);

  app.controller('DetailModalCtrl', [
    '$scope', '$rootScope', '$routeParams', 'GUNS', 'FAMILIES', 'PRINTING', '_', '$mdDialog', 'gun', function($scope, $rootScope, $routeParams, GUNS, FAMILIES, PRINTING, _, $mdDialog, gun) {
      $scope.gun = gun;
      $scope.hide = function() {
  ...
  app.factory('CRITERIA', [
    '_', 'GUNS', function(_, GUNS) {
      var category, criteria, gun, index, name, _i, _j, _len, _len1, _ref;
      criteria = {};
      criteria.categories = [
        {
          "display": "Caliber",
      ...
      */
      ...
      _ref = criteria.categories;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        category = _ref[_i];
        category.members = [];
        name = category.field;
        for (_j = 0, _len1 = GUNS.length; _j < _len1; _j++) {
          gun = GUNS[_j];
          index = _.findIndex(criteria.categories, {
            'field': name
          });
  ...
  */
  ...
  dataThing.factory('GUNS', [
    'PRINTING', 'FAMILIES', '$sce', function(PRINTING, FAMILIES, $sce) {
      var base, baseIndex, baseProperties, family, findex, gun, gunList, lookup, member, mindex, option, product, prop, variants, video, zzz, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref, _ref1, _ref2, _ref3, _ref4;
      gunList = [
        {
          "id": "CZ-75B9",
          "manufacturer": "CZ",
          ...
          "options": ["safety", "firing pin"],
          "youtube": ["https://www.youtube.com/embed/WgDhURHa0OQ", "https://www.youtube.com/embed/ybT3wCzzB0M"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B005SBMU4K&asins=B005SBMU4K&linkId=J3PSGD2XPJWGWK6P&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B00KZENW2O&asins=B00KZENW2O&linkId=RXFF2MFFIVLWJDIZ&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B00MTY9S1C&asins=B00MTY9S1C&linkId=M2FL3XJRGLMFL4ZD&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B000S5SCPM&asins=B000S5SCPM&linkId=WMH76CSUHY4E2YUN&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B007PAK79C&asins=B007PAK79C&linkId=5FRFYRGNTJNHB5JH&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B00TE3P0R2&asins=B00TE3P0R2&linkId=24T5RNV6TPDIZYBC&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B007ZSKQU4&asins=B007ZSKQU4&linkId=GK7IF3G2QLN6G52T&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "CZ-75B40",
          "variant": "CZ-75B9",
          "name": "75 B",
          ...
          "capacity": 10,
          "options": ["safety", "firing pin"],
          "youtube": ["https://www.youtube.com/embed/PWkYoPhQMwk", "https://www.youtube.com/embed/zdXIhHhDpbM", "https://www.youtube.com/embed/hsFuqbfiWx4"]
        }, {
          "id": "CZ-75BD",
          "variant": "CZ-75B9",
          "name": "75 BD",
          ...
          "options": ["decocker", "firing pin"],
          "youtube": ["https://www.youtube.com/embed/eSlV4BF4ABE"]
        }, {
          "id": "CZ-75BSA9",
          "variant": "CZ-75B9",
          "name": "75 B SA",
          ...
          "trigger": "hammer SA",
          "youtube": ["https://www.youtube.com/embed/BvXbvar8H3c", "https://www.youtube.com/embed/jSM2ENTlNFI"]
        }, {
          "id": "CZ-75BSA40",
          "variant": "CZ-75BSA9",
          "name": "75 B SA",
          ...
          "options": ["rail", "safety", "firing pin"],
          "youtube": ["https://www.youtube.com/embed/WgDhURHa0OQ", "https://www.youtube.com/embed/ybT3wCzzB0M"]
        }, {
          "id": "CZ-75Bthreaded",
          "manufacturer": "CZ",
          ...
          "options": ["safety", "firing pin", "threaded"],
          "youtube": ["https://www.youtube.com/embed/xhAA1w3R0LU"]
        }, {
          "id": "CZ-75Bthreaded-ns",
          "variant": "CZ-75Bthreaded",
          "name": "75 B Ω",
          "subname": "Suppressor-Ready, decocker",
          "options": ["decocker", "firing pin", "threaded"]
        }, {
          "id": "Glock17-4",
          "manufacturer": "Glock",
          "name": "17",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/zkIGtwYwmmk", "https://www.youtube.com/embed/V0-GcOZ3Hrc", "https://www.youtube.com/embed/nswOxj0RT2s"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B016ISRE88&asins=B016ISRE88&linkId=MCB4IHD36N4MAGVN&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B009KQ5R94&asins=B009KQ5R94&linkId=FGUJO3V7XYB4MWAL&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock19-4",
          "manufacturer": "Glock",
          "name": "19",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/nswOxj0RT2s", "https://www.youtube.com/embed/V0-GcOZ3Hrc", "https://www.youtube.com/embed/3z2w5VlPo3Y"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock26-4",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/nswOxj0RT2s", "https://www.youtube.com/embed/3wHUEwWmklE"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock22-4",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/R1ePVd6hKeE", "https://www.youtube.com/embed/jolIaRoOI_g", "https://www.youtube.com/embed/ODt-bW1eD5M"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B016ISRE88&asins=B016ISRE88&linkId=MCB4IHD36N4MAGVN&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B009KQ5R94&asins=B009KQ5R94&linkId=FGUJO3V7XYB4MWAL&show_border=true&link_opens_in_new_window=true", "//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock23-4",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/Tg6RURKEi5Q", "https://www.youtube.com/embed/zyX6KCBLbsY"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock27-4",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/68epOKeo7M8", "https://www.youtube.com/embed/hdyq1p7UVAw", "https://www.youtube.com/embed/3wHUEwWmklE"],
          "amazon": ["//ws-na.amazon-adsystem.com/widgets/q?ServiceVersion=20070822&OneJS=1&Operation=GetAdHtml&MarketPlace=US&source=ac&ref=tf_til&ad_type=product_link&tracking_id=pistsear-20&marketplace=amazon&region=US&placement=B004AO6BUK&asins=B004AO6BUK&linkId=FD5ZLEASLTOIHHRP&show_border=true&link_opens_in_new_window=true"]
        }, {
          "id": "Glock20-4",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/kUyf1y50lC4", "https://www.youtube.com/embed/yamL7SjaGjE", "https://www.youtube.com/embed/-m0satEkGjw", "https://www.youtube.com/embed/uCrAaomJS_M"]
        }, {
          "id": "Glock29-4",
          "manufacturer": "Glock",
          "name": "29",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/jaZvjVgy0MM", "https://www.youtube.com/embed/VCvJe6QEWNQ", "https://www.youtube.com/embed/gVQTgiDyR5k"]
        }, {
          "id": "Glock21-4",
          "manufacturer": "Glock",
          "name": "21",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/jo317hYV-ag", "https://www.youtube.com/embed/Eynv1q_Ttb0", "https://www.youtube.com/embed/iOIHgwVDy-U", "https://www.youtube.com/embed/vvHZFNGz7xg"]
        }, {
          "id": "Glock30-4",
          "manufacturer": "Glock",
          "name": "30",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/m3b5xuTwZHM", "https://www.youtube.com/embed/vvHZFNGz7xg", "https://www.youtube.com/embed/5Px_8OPx0ZA"]
        }, {
          "id": "Glock37-4",
          "manufacturer": "Glock",
          "name": "37",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/q_fCmzIw1WA", "https://www.youtube.com/embed/BPbfPmFhWyY"]
        }, {
          "id": "Glock38",
          "manufacturer": "Glock",
          "name": "38",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/THsIGbgoYiY", "https://www.youtube.com/embed/BPbfPmFhWyY"]
        }, {
          "id": "Glock39",
          "manufacturer": "Glock",
          "name": "39",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/BPbfPmFhWyY"]
        }, {
          "id": "SW-MP22c",
          "manufacturer": "S&W",
          "name": "M&P 22 Compact",
          ...
          "options": ["magazine safety", "safety", "rail", "threaded"],
          "youtube": ["https://www.youtube.com/embed/W5gRlIQy_MA", "https://www.youtube.com/embed/EpZx0kpY7SI"]
        }, {
          "id": "SW-MPshield40",
          "manufacturer": "S&W",
          "name": "M&P Shield",
          ...
          "options": [],
          "youtube": ["https://www.youtube.com/embed/O81wSD7mg0c", "https://www.youtube.com/embed/EqBoZaOr33k"]
        }, {
          "id": "SW-MPshield9",
          "manufacturer": "S&W",
          "name": "M&P Shield",
          ...
          "options": [],
          "youtube": ["https://www.youtube.com/embed/aWWSpVn7NT8", "https://www.youtube.com/embed/EqBoZaOr33k"]
        }, {
          "id": "sw-mp-bodyguard",
          "manufacturer": "S&W",
          "name": "M&P Bodyguard",
          ...
          "options": ["safety"],
          "youtube": ["https://www.youtube.com/embed/fzSS9GzEBd4", "https://www.youtube.com/embed/LT81_h980gY"]
        }, {
          "id": "sw-sd40ve",
          "manufacturer": "S&W",
          "name": "SD40 VE",
          ...
          "options": ["trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/1cleMs8xmqc", "https://www.youtube.com/embed/3rMsAqtQO9s", "https://www.youtube.com/embed/6khWQ6PbyeE"]
        }, {
          "id": "sw-sd9ve",
          "manufacturer": "S&W",
          "name": "SD9 VE",
          ...
          "options": ["trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/xpW7rdTJpBE", "https://www.youtube.com/embed/7thRJU-wz5s"]
        }, {
          "id": "sw-1911-9",
          "manufacturer": "S&W",
          "name": "SW1911",
          ...
          "options": ["safety", "grip"],
          "youtube": ["https://www.youtube.com/embed/zv_xcebzXyI"]
        }, {
          "id": "sw-1911pro",
          "variant": "sw-1911-9",
          ...
          "youtube": ["https://www.youtube.com/embed/TgY1oKF0cbE", "https://www.youtube.com/embed/zrBF4MFNe4s"]
        }, {
          "id": "sw-1911-45",
          "variant": "sw-1911-9",
          "name": "SW1911",
          ...
          "caliber": ".45 ACP",
          "capacity": 8,
          "youtube": ["https://www.youtube.com/embed/QSS8tI1vnYY", "https://www.youtube.com/embed/5mGC-ZxggHA"]
        }, {
          "id": "sw-1911-45-3",
          "variant": "sw-1911-45",
          "name": "SW1911",
          ...
          "youtube": ["https://www.youtube.com/embed/W44cdYF2RXg"]
        }, {
          "id": "sw-model41",
          "manufacturer": "S&W",
          "name": "Model 41",
          ...
          "options": ["safety"],
          "youtube": ["https://www.youtube.com/embed/a_kzs01U1l8", "https://www.youtube.com/embed/yAVE9qGYm1g"]
        }, {
          "id": "sw-mp40",
          "manufacturer": "S&W",
          "name": "M&P 40",
          ...
          "options": ["safety", "trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/hrFkXCUwGDM", "https://www.youtube.com/embed/hZZad6KmfMY"]
        }, {
          "id": "sw-mp40c",
          "manufacturer": "S&W",
          "name": "M&P Compact .40",
          ...
          "options": ["trigger", "safety", "rail"],
          "youtube": ["https://www.youtube.com/embed/OLFAt63aNbY", "https://www.youtube.com/embed/pvBVCQBUnLs"]
        }, {
          "id": "sw-mp40c-ns",
          "variant": "sw-mp40c",
          "name": "M&P Compact .40",
          ...
          "options": ["safety", "trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/sxXQtLMNGqI", "https://www.youtube.com/embed/piDK0xjlLGs"]
        }, {
          "id": "sw-mp45-4",
          "variant": "sw-mp45",
          "name": "M&P45",
          ...
          "options": ["trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/piDK0xjlLGs", "https://www.youtube.com/embed/4UBhQpvEPTU"]
        }, {
          "id": "sw-mp45c-ns",
          "variant": "sw-mp45c",
          "name": "M&P45c",
          ...
          "options": ["trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/MtuGiBSm4Vs"]
        }, {
          "id": "sw-mp357",
          "manufacturer": "S&W",
          "name": "M&P357",
          ...
          "options": ["trigger", "thumb", "rail"],
          "youtube": ["https://www.youtube.com/embed/tk3b42A-NQQ"]
        }, {
          "id": "sw-mp9",
          "manufacturer": "S&W",
          "name": "M&P9",
          ...
          "options": ["thumb", "trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/Dh1nsoPxE-Q"]
        }, {
          "id": "sw-mp9-ns",
          "variant": "sw-mp9",
          "name": "M&P9",
          ...
          "options": ["thumb", "trigger", "rail"],
          "youtube": ["https://www.youtube.com/embed/WFv3W8TOw1A", "https://www.youtube.com/embed/6V7CogUc4nk"]
        }, {
          "id": "sw-mp9c-ns",
          "variant": "sw-mp9c",
          "name": "M&P9c",
          ...
          "options": ["safety", "decocker"],
          "youtube": ["https://www.youtube.com/embed/S4kkckLFl5g", "https://www.youtube.com/embed/_BOMmIvMRm8"]
        }, {
          ...
          "options": ["firing pin block", "decocker"],
          "youtube": ["https://www.youtube.com/embed/S4kkckLFl5g", "https://www.youtube.com/embed/_BOMmIvMRm8"]
        }, {
          "id": "beretta-m9a3",
          "manufacturer": "Beretta",
          ...
          "options": ["firing pin block", "decocker", "threaded", "safety", "rail"],
          "youtube": ["https://www.youtube.com/embed/jincc8cQtDo", "https://www.youtube.com/embed/HkQJca15CYk"]
        }, {
          "id": "beretta-m9a3-ns",
          ...
          "subname": "no safety",
          "image": "/img/beretta-m9a3.jpg",
          "options": ["firing pin block", "decocker", "threaded", "rail"]
        }, {
          "id": "beretta-96a1",
          "variant": "beretta-92fs",
          "name": "96A1",
          ...
          "caliber": ".40 S&W",
          "capacity": 12,
          "options": ["firing pin block", "decocker", "safety", "rail"],
          "youtube": ["https://www.youtube.com/embed/c6GiamFFjME"]
        }, {
          "id": "beretta-m9-22",
          "manufacturer": "Beretta",
          "name": "M9-22",
          ...
          "options": ["safety", "decocker"],
          "youtube": ["https://www.youtube.com/embed/TY4s9HVhu44"]
        }, {
          "id": "ruger-lcp",
          "manufacturer": "Ruger",
          "name": "LCP",
          ...
          "options": [],
          "youtube": ["https://www.youtube.com/embed/QrE371hU8m4", "https://www.youtube.com/embed/EvVi5DQF9X4", "https://www.youtube.com/embed/e9FwISjVuT4"]
        }, {
          "id": "ruger-lc9s",
          "manufacturer": "Ruger",
          "name": "LC9s",
          ...
          "options": ["safety", "magazine safety"],
          "youtube": ["https://www.youtube.com/embed/Opx_IFm0iQU"]
        }, {
          "id": "ruger-lc9s-pro",
          "manufacturer": "Ruger",
          "name": "LC9s Pro",
          ...
          "options": [],
          "youtube": ["https://www.youtube.com/embed/p2bBSHxk-Oc"]
        }, {
          "id": "ruger-lc380",
          "manufacturer": "Ruger",
          "name": "LC380",
          ...
          "options": [],
          "youtube": ["https://www.youtube.com/embed/EXy9Nd2InyI", "https://www.youtube.com/embed/6XycvZKmcTs"]
        }, {
          "id": "ruger-9e",
          "manufacturer": "Ruger",
          "name": "9E",
          ...
          "options": ["safety", "rail"],
          "youtube": ["https://www.youtube.com/embed/a2zlfdqTNr0", "https://www.youtube.com/embed/3R2Usxu7fvk", "https://www.youtube.com/embed/R8XCCDV4O1w"]
        }, {
          "id": "ruger-sr9",
          "manufacturer": "Ruger",
          "name": "SR9",
          ...
          "options": ["safety", "rail"],
          "youtube": ["https://www.youtube.com/embed/ysokL6pHf1I", "https://www.youtube.com/embed/apzCB-gLAPg", "https://www.youtube.com/embed/E1YMWh94L84"]
        }, {
          "id": "ruger-sr9c",
          "manufacturer": "Ruger",
          "name": "SR9c",
          ...
          "options": ["safety", "rail", "magazine safety"],
          "youtube": ["https://www.youtube.com/embed/zT1q1C_PnIY", "https://www.youtube.com/embed/XjP_Mj4P4Y8", "https://www.youtube.com/embed/vyP5gEjQ58g", "https://www.youtube.com/embed/lpj9Pwv6TTY"]
        }, {
          "id": "ruger-sr40",
          "manufacturer": "Ruger",
          "name": "SR40",
          ...
          "options": ["safety", "rail", "magazine safety"],
          "youtube": ["https://www.youtube.com/embed/mYDYE9kynbI", "https://www.youtube.com/embed/1XVrBv7ldOs"]
        }, {
          "id": "ruger-sr40c",
          "manufacturer": "Ruger",
          "name": "SR40c",
          ...
          "options": ["safety", "rail", "magazine safety"],
          "youtube": ["https://www.youtube.com/embed/LNlqwm6_ol8", "https://www.youtube.com/embed/ilpj82qwJQY", "https://www.youtube.com/embed/mElslmZN9g4"]
        }, {
          "id": "ruger-sr45",
          "manufacturer": "Ruger",
          "name": "SR45",
          ...
          "options": ["safety", "rail", "magazine safety"],
          "youtube": ["https://www.youtube.com/embed/R4LVmk1rPNw", "https://www.youtube.com/embed/YsFr8ZFSQOw", "https://www.youtube.com/embed/BoqIJr0V3mw"]
        }, {
          "id": "ruger-american-9",
          "manufacturer": "Ruger",
          "name": "American",
          ...
          "options": ["rail"],
          "youtube": ["https://www.youtube.com/embed/FEGqYYgSljE"]
        }, {
          ...
          "options": ["rail"],
          "youtube": ["https://www.youtube.com/embed/NKUAxAc6NWk"]
        }, {
          ...
          "options": ["safety", "grip safety"],
          "youtube": ["https://www.youtube.com/embed/CAZ5EYxpl2o"]
        }, {
          ...
          "options": ["safety", "rail", "decocker"],
          "youtube": ["https://www.youtube.com/embed/FWNRG7xBPPk"]
        }, {
          "id": "glock-43",
          "manufacturer": "Glock",
          "name": "43",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/c5BznRK2dRA", "https://www.youtube.com/embed/H6CVf1TKZyE", "https://www.youtube.com/embed/8pHdg4F15aQ"]
        }, {
          "id": "glock-42",
          "manufacturer": "Glock",
          "name": "42",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/TevjYfFe49s", "https://www.youtube.com/embed/LskihWv3ALw", "https://www.youtube.com/embed/_ZOkPAET86c"]
        }, {
          "id": "glock-36",
          "manufacturer": "Glock",
          "name": "36",
          ...
          "options": ["trigger", "firing pin", "drop"],
          "youtube": ["https://www.youtube.com/embed/6dW9fu8kmZI", "https://www.youtube.com/embed/hLCtDtaWSnw", "https://www.youtube.com/embed/7O-GHoHqqsM", "https://www.youtube.com/embed/qprvXMVGqGw", "https://www.youtube.com/embed/bYJgIFQIAy4"]
        }, {
          "id": "glock-34",
          "manufacturer": "Glock",
          "name": "34",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/BTJfnuhKBc4", "https://www.youtube.com/embed/unh_RoCkjz8", "https://www.youtube.com/embed/gPW-sfPWzOo"]
        }, {
          "id": "glock-35",
          "manufacturer": "Glock",
          "name": "35",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/jTSAKHPRrl0", "https://www.youtube.com/embed/oiBpg71Bul8", "https://www.youtube.com/embed/fwNiTLk91X4", "https://www.youtube.com/embed/vrnnwlJ2vT8"]
        }, {
          "id": "glock-41",
          "manufacturer": "Glock",
          "name": "41",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/eStUpDS-8wc", "https://www.youtube.com/embed/95YAUBUK4rA", "https://www.youtube.com/embed/dbqovRFSjV0"]
        }, {
          "id": "glock-17L",
          "manufacturer": "Glock",
          "name": "17L",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/UYuPEfPb_wc", "https://www.youtube.com/embed/tHTBPEtrDO8", "https://www.youtube.com/embed/OPp0N1wtz10"],
          "amazon": []
        }, {
          "id": "glock-24",
          "manufacturer": "Glock",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/vCwT7K7sPKQ", "https://www.youtube.com/embed/dd848bus9aE"]
        }, {
          "id": "glock-40",
          "manufacturer": "Glock",
          "name": "40",
          ...
          "options": ["trigger", "firing pin", "drop", "rail"],
          "youtube": ["https://www.youtube.com/embed/lwGrpePCYeI", "https://www.youtube.com/embed/0LUJZ1qWUrM", "https://www.youtube.com/embed/Lvt2pxSGABg"]
        }
      ];
      for (_i = 0, _len = gunList.length; _i < _len; _i++) {
        gun = gunList[_i];
        if (gun.variant) {
          baseIndex = _.findIndex(gunList, {
            "id": gun.variant
          });
          if (baseIndex === -1) {
        ...
        if (gun.options) {
          ...
          for (_m = 0, _len4 = _ref2.length; _m < _len4; _m++) {
            option = _ref2[_m];
            lookup = _.find(PRINTING.options, {
              "name": option
            });
            if (lookup != null) {
              gun.printOptions.push(lookup.display);
            } else {
              gun.printOptions.push(option);
            }
          }
        }
        zzz = _.filter(FAMILIES, function(f) {
          return _.includes(f.members, gun.id);
        });
        gun.families = _.cloneDeep(zzz);
        _ref3 = gun.families;
        for (findex = _n = 0, _len5 = _ref3.length; _n < _len5; findex = ++_n) {
          family = _ref3[findex];
          _ref4 = family.members;
          for (mindex = _o = 0, _len6 = _ref4.length; _o < _len6; mindex = ++_o) {
            member = _ref4[mindex];
            gun.families[findex].members[mindex] = _.find(gunList, {
              "id": member
            });
          }
        }
        variants = {
          "name": "Variants",
          "members": _.filter(gunList, {
            'variant': gun.variant || gun.id
          })
        };
        if (gun.variant) {
          variants.members.push(_.find(gunList, {
            'id': gun.variant
          }));
        }
        if (variants.members.length > 0) {
          gun.families.push(variants);
        }
      }
      return gunList;
    }
  ...
  app.factory('FAMILIES', function() {
    return [
      {
        "name": "9mm Glocks",
        "members": ["Glock17-4", "Glock19-4", "Glock26-4"]
      }, {
        "name": ".40 Glocks",
        "members": ["Glock22-4", "Glock23-4", "Glock27-4"]
      }, {
        "name": "10mm Glocks",
        "members": ["Glock20-4", "Glock29-4"]
      }, {
        "name": ".45 Glocks",
        "members": ["Glock21-4", "Glock30-4"]
      }, {
        "name": ".45 GAP Glocks",
        "members": ["Glock37-4", "Glock38", "Glock39"]
      }
    ];
  });
  ...
  app.factory('UserSearch', [
    '_', 'CRITERIA', function(_, CRITERIA) {
      var self;
      self = {};
      self.power = {
        categories: [[], [], [], []],
